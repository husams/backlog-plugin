...
def cmd_dep_add(ctx: Ctx, args) -> int:
    ...
    row = deps.add(
        ctx.conn, ctx.pid, a, b, kind, note=args.note or "", actor=args.actor
    )
...
def cmd_dep_list(ctx: Ctx, args) -> int:
    if args.key:
        ...
        rows = [
            ...
            for e in sorted(
                edges,
                key=lambda e: (e["kind"] != "blocks", e["direction"], e["other_key"]),
            )
        ]
...
def cmd_dep_check(ctx: Ctx, args) -> int:
    ...
    ok = not blockers
    text = f"{task['key']}  " + (
        "READY — nothing is blocking it"
        if ok
        else "BLOCKED\n"
        + table(
            ["KEY", "STATUS", "TITLE"],
            [[b["other_key"], b["other_status"], b["other_title"]] for b in blockers],
        )
    )
...
def cmd_dep_graph(ctx: Ctx, args) -> int:
    ...
    if args.format == "json" or ctx.json:
        ...
        return 0
    text = table(
        ["FROM", "KIND", "TO"], [[e["from_key"], e["kind"], e["to_key"]] for e in edges]
    )
    if blocked:
        text += "\n\ncurrently blocked:\n" + "\n".join(
