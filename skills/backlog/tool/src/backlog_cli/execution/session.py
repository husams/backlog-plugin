"""Thin public validation API passthroughs."""
...
def set_item_execution(self, item_id: int, spec: dict) -> dict:
    ...
    return store.set_executable(self._conn, item_id, spec)
...
def record_execution_result(
    ...
    **kwargs,
    ...
    return audit.record_result(self._conn, item_id, spec_fingerprint, status, **kwargs)


def execution_history(
    self, item_id: int, *, limit: int = 20, project_root=None
) -> list[dict]:
    """Newest-first bounded validation history with freshness metadata."""
    from pathlib import Path

    root = Path(project_root) if project_root is not None else None
    return audit.execution_history(self._conn, item_id, limit=limit, project_root=root)


def waive_validation(
    self, item_id: int, *, reason: str, actor: str | None = None
) -> dict:
    """Audit an explicit waiver for the item's current execution spec."""
    return audit.waive_validation(
        self._conn,
        self.pid,
        item_id,
...
def execution_policy(self, project_root) -> ExecutionPolicy:
    ...
    return policy.load_policy(Path(project_root))
...
def source_identity(self, project_root) -> SourceIdentity:
    ...
    return policy.source_identity(Path(project_root))
...
def run_hook_validation(
    ...
    *,
    ...
    return hook_runner.run_hook_validation(
        self,
        item_id,
        actor=actor or self.actor or "unknown",
