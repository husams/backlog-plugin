...
def pre_transition(
    ...
    if not callable(callback):
        raise BacklogError(f"{backlog_dir / 'hooks.py'}: pre_transition is not callable")
    try:
...
def post_transition(
    ...
    if callback is None:
        return
    if not callable(callback):
        raise BacklogError(f"{backlog_dir / 'hooks.py'}: post_transition is not callable")
    try:
